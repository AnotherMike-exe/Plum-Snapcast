...
6. **AirPlay Notification Guards**: Each `Plugin.Stream.Player.Properties` notification triggers `onResync()` on all snapclients (~50–120 ms gap). This is a **known, unfixed upstream Snapcast bug** ([snapcast#1351](https://github.com/snapcast/snapcast/issues/1351), [#1318](https://github.com/snapcast/snapcast/issues/1318), [#433](https://github.com/badaix/snapcast/issues/433)) — an individual push can't be made cheap, so we only minimise how many we send. `airplay-control-script.py` applies five guards: volume debounce (500 ms), track-change pause guard (2 s), resume suppression (skip the redundant `playing→playing` after a guarded track change), metadata debounce (400 ms), and metadata content-dedup. Do not remove these without understanding the resync impact. **Residuals that are NOT control-script-fixable:** a different-song skip still sends one metadata push (one blip) — only removable by moving metadata off Snapcast Properties onto our own API like `playback_api.py` did for position; and the AirPlay buffer-flush gap on skip is a physical audio discontinuity inherent to the protocol. See `docs/ARCHITECTURE.md` for details.
